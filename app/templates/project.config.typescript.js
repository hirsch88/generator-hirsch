module.exports = function (isGenerator) {
  ...
  var bowerFilesCss = [];
  try {
    bowerFilesTs = (!isGenerator) ? wiredep({})['ts'] : [];
  ...
  function getAngularScripts() {
    return {
      files: [
        ...
        path.join(projectConfig.path.srcDir, projectConfig.path.app.coreDir, '*.ts'),
        path.join(projectConfig.path.srcDir, projectConfig.path.app.commonDir, '*.ts'),
        path.join(projectConfig.path.srcDir, projectConfig.path.app.commonDir, '**/*.ts'),
